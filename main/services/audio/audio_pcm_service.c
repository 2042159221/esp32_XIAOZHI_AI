...
{
    (void)arg;
    uint8_t *frame = (uint8_t *)malloc(AUDIO_PCM_FRAME_BYTES);
    if (frame == NULL) {
        ESP_LOGE(TAG, "alloc capture frame failed");
    ...
    while (s_stream_running) {
        int ret = esp_codec_dev_read(bsp_audio_get_codec(), frame, AUDIO_PCM_FRAME_BYTES);
        if (ret != ESP_CODEC_DEV_OK) {
        ...
        if (s_tx_cb != NULL) {
            esp_err_t err = s_tx_cb(frame, AUDIO_PCM_FRAME_BYTES, s_tx_user_ctx);
            if (err != ESP_OK) {
...
esp_err_t audio_pcm_service_init(void)
{
    ...
    if (s_playback_rb == NULL) {
        s_playback_rb = xRingbufferCreate(AUDIO_PCM_RING_BYTES, RINGBUF_TYPE_BYTEBUF);
...
esp_err_t audio_pcm_service_local_loopback(uint32_t duration_ms)
{
    ...
    uint8_t *frame = (uint8_t *)malloc(AUDIO_PCM_FRAME_BYTES);
    ESP_RETURN_ON_FALSE(frame != NULL, ESP_ERR_NO_MEM, TAG, "alloc loopback frame failed");
    ...
    while (remaining_ms > 0 && s_loopback_running) {
        int ret = esp_codec_dev_read(bsp_audio_get_codec(), frame, AUDIO_PCM_FRAME_BYTES);
        if (ret == ESP_CODEC_DEV_OK) {
            ret = esp_codec_dev_write(bsp_audio_get_codec(), frame, AUDIO_PCM_FRAME_BYTES);
        }
        if (ret != ESP_CODEC_DEV_OK) {
