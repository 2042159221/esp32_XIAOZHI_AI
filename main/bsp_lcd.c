...
static esp_err_t bsp_lcd_init_backlight(void)
{
    ...
    ESP_RETURN_ON_ERROR(gpio_set_level(BSP_LCD_PIN_NUM_BK_LIGHT, EXAMPLE_LCD_BK_LIGHT_OFF_LEVEL), TAG, "turn off LCD backlight failed");
...
{
    if (s_backlight_initialized) {
        gpio_set_level(BSP_LCD_PIN_NUM_BK_LIGHT, EXAMPLE_LCD_BK_LIGHT_ON_LEVEL);
    }
}
...
{
    if (s_backlight_initialized) {
        gpio_set_level(BSP_LCD_PIN_NUM_BK_LIGHT, EXAMPLE_LCD_BK_LIGHT_OFF_LEVEL);
    }
}
