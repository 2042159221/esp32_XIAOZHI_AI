...
static void app_display_clear_center_area(void)
{
    if (s_qrcode != NULL) {
        lv_obj_del(s_qrcode);
        s_qrcode = NULL;
...
static void app_display_create_base_screen(void)
{
...
esp_err_t app_display_init(void)
{
    ...
    lvgl_port_lock(0);
    app_display_create_base_screen();
    lvgl_port_unlock();
...
void app_display_show_ble_qrcode(const char *payload)
{
    ...
    if (payload == NULL || payload[0] == '\0') {
        ESP_LOGW(TAG, "BLE provisioning payload is empty");
        app_display_show_message("BLE Provisioning", "QR payload is empty");
        return;
    }
    ...
    if (s_qrcode == NULL) {
        ...
        app_display_show_message("BLE Provisioning", "Failed to create QR code");
        return;
    }

    lv_qrcode_set_size(s_qrcode, APP_DISPLAY_QRCODE_SIZE);
    lv_qrcode_set_dark_color(s_qrcode, lv_color_hex(APP_DISPLAY_QRCODE_DARK_COLOR));
    lv_qrcode_set_light_color(s_qrcode, lv_color_hex(APP_DISPLAY_QRCODE_LIGHT_COLOR));
    lv_qrcode_set_quiet_zone(s_qrcode, true);
    lv_obj_set_style_border_color(s_qrcode, lv_color_hex(APP_DISPLAY_QRCODE_BORDER_COLOR), 0);
    ...
    if (lv_qrcode_update(s_qrcode, payload, strlen(payload)) != LV_RESULT_OK) {
        ...
        app_display_show_message("BLE Provisioning", "QR update failed. Check serial log");
        return;
    }
